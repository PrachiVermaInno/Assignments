...
# ---- Savings Account ----
class SavingsAccount(Account): #inheritence/ compile time polymorphism
    def account_type(self):
        return "Savings"
...
class BankSystem: # Encapsulation
    def __init__(self):
    ...
    def create_account(self, acc_no, name, acc_type):
        if acc_type == "savings":
            self.accounts[acc_no] = SavingsAccount(acc_no, name)
        elif acc_type == "current":
            self.accounts[acc_no] = CurrentAccount(acc_no, name)
        else:
            print("Invalid account type!")
            return
        print(f"{acc_type.capitalize()} Account Created for {name} with Acc No {acc_no}")
